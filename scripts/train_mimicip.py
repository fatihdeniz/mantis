...
exp, model, loader = train(nodes_file, edges_file, model_file, feature_labels, args1)
# loader = Loader(nodes_file, edges_file, feature_labels, **coeffs)
# exp = SAGE_Experiment(loader.data, **coeffs)
cleanAccs = test(exp, loader, curr, i)
cleanAccs['clp'] = i
cleanAccs['type'] = 'clean'
result.append(cleanAccs)

advAccs = exp.advtest(model, loader.data, loader.test_mask, clipLmt=clp, epsilon=eps)
advAccs['clp'] = i
advAccs['type'] = 'adv'
...
result_df.to_csv(f'/export/sec02/fatih/paper_stats/data/attack/mimicip_{i}.csv', index=False)
